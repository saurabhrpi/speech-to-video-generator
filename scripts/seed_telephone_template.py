"""Seed the Firestore template registry with the Telephone fixture (S79).
...
no CATEGORY_LABEL_OVERRIDES entry needed). Fourth girl_dances row template.

Preview note (S79): preview generated kling-v2-6 + pro + video orientation.
Subject = different young woman (Pattern B), teal/green athletic bralette +
black leggings + sneakers, spacious sunlit loft (S78 roomy composition). Driver
trimmed to start at t=0.5s; Kling output trimmed to 14.0s (dropped the
end-of-clip duplicate). No audio-lead correction needed (audio landed on lips).
...
Re-running overwrites the Telephone doc.

Usage:
    .venv/bin/python scripts/seed_telephone_template.py
"""
...
log = logging.getLogger("seed_telephone_template")


TEMPLATE_ID = "viral-dances-telephone"
...
_BASE = "https://assets.speech-2-video.ai/viral-dances/telephone"
...
TELEPHONE_FIXTURE = {
    ...
    "title": "Telephone",
    "description": "Stop callin', stop callin'. 📞💃",
...
def main() -> None:
    ...
    written = upsert_template(TEMPLATE_ID, TELEPHONE_FIXTURE)
    log.info(
