"""Seed the Firestore template registry with the Boot Stop Working fixture (S75).

Sister to scripts/seed_rasputin_template.py. Pipeline A
(motion-onto-character via Kling Motion Control), use_nbp_regen=true with
the generic full-body framing hint.

Category: iconic_dances (new row introduced S75 — mechanically title-cases
to "Iconic Dances" in mobile, no CATEGORY_LABEL_OVERRIDES entry needed).

Starts as DRAFT — flip to published via `scripts/set_template_status.py`
once sim-tested E2E with a real selfie.

Re-running overwrites the Boot Stop Working doc.

Usage:
    .venv/bin/python scripts/seed_boot_stop_working_template.py
"""
...
log = logging.getLogger("seed_boot_stop_working_template")


TEMPLATE_ID = "viral-dances-boot-stop-working"
...
BOOT_STOP_FIXTURE = {
    ...
    "title": "Boot Stop Working",
    "description": "Boots on, smile up, ride out. 🤠",
    "published_status": STATUS_DRAFT,
    "assets": {
        "driving_video_url": "https://assets.speech-2-video.ai/viral-dances/boot-stop-working/driving_video.mp4",
        "scene_image_url": None,
        "thumbnail_url": None,
        "preview_video_url": "https://assets.speech-2-video.ai/viral-dances/boot-stop-working/preview_video.mp4",
    },
...
def main() -> None:
    ...
    written = upsert_template(TEMPLATE_ID, BOOT_STOP_FIXTURE)
    log.info(
