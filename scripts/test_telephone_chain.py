"""V2 Pipeline A Telephone template — NBP-edit → Kling Motion Control chain (S79).

Fourth template of the "Girl Dances" row (category girl_dances).
Sister to scripts/test_buttons_chain.py and scripts/test_river_chain.py.

Telephone is an UPRIGHT, camera-facing standing dance (expressive arm gestures,
shot waist-up in a bright bedroom) — Kling-trackable, unlike River's floor work.
So this uses the runbook's S78 standing composition: roomy/wider full-body,
centered, feet+floor visible, generous lateral room for arm/dance movement.

Pattern B (different woman) — the source is an identifiable competitor-app
dancer; we swap in a genuinely different subject. Styling: match the source's
register (fitted dance bralette + leggings, bright domestic scene) but in a
clearly different color + a more spacious interior.

Source: Telephone_start_at_0.5_sec.mov (1284×1388, ~1:1, ~15.2s; trim the first
0.5s for the driver). Reference frame = clip t=2s (telephone_frame_t2.png) — the
clearest front-facing face (later frames have head tilt / motion blur).

Usage:
    .venv/bin/python scripts/test_telephone_chain.py --no-kling
    .venv/bin/python scripts/test_telephone_chain.py --edited-image ~/Downloads/telephone_edit_xxx.png --keep-audio
"""
...
DEFAULT_REFERENCE = Path(
    "/Users/saurabhsmacbookair/Downloads/App Templates Prep/Working/Girl Dances/telephone_frame_t2.png"
)

TELEPHONE_EDIT_PROMPT = (
    "Edit this image:\n"
    ...
    "from the input:\n"
    "  - Hair: long hair, but a DIFFERENT color from the input's dark "
    "brown — e.g. honey-blonde, warm caramel, auburn, or light golden-brown — "
    "pick one.\n"
    ...
    "hair.\n"
    "- Wardrobe: match the input's register and bralette cut, but in a clearly "
    "DIFFERENT color — a fitted athletic BRALETTE (a sports-bra-style crop that "
    "sits above the midriff, same coverage as the input's bralette) with slim "
    "leggings and sneakers, in a color that is NOT the input's maroon/burgundy "
    "(e.g. deep teal, emerald, lavender, or champagne — pick one). Tasteful; do "
    "NOT make it more revealing than the input's bralette. No readable text or "
    "logos.\n"
    "- Scene: match the input's bright domestic register but in a SPACIOUS, "
    "airy interior — a large, uncluttered, sunlit room (e.g. a bright "
    "minimalist living space, a sunlit loft, or an open studio — NOT a cramped "
    "bedroom corner), with soft natural daylight and a WIDE expanse of clear, "
    "open, uncluttered floor extending well to her LEFT AND RIGHT. Nothing "
    "within arm's reach on either side.\n"
    ...
    "central 55-65% of the frame height, and CENTERED horizontally. Leave "
    "GENEROUS, roughly EQUAL open empty floor on BOTH the left and right sides, "
    "plus clear floor below the feet — room for lateral dance movement in "
    "either direction.\n"
    ...
    "feet at least 8-10% of frame height above the bottom.\n"
...
# S77 flow: the chain drives off the trimmed RAW source (raw_source.mp4); the
# high-bitrate Kling output is later uploaded as driving_video.mp4 (runtime driver).
TELEPHONE_DRIVING_VIDEO = (
    "https://assets.speech-2-video.ai/viral-dances/telephone/raw_source.mp4"
)
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "telephone_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False,
              driving_video: str = TELEPHONE_DRIVING_VIDEO) -> int:
    settings = get_settings()
    selfies_bucket = settings.r2_selfies_bucket
    key = f"spike-outputs/telephone-chain/{uuid.uuid4().hex}.png"
    ...
    out_path = out_dir / f"telephone_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument("--driving-video", default=TELEPHONE_DRIVING_VIDEO,
