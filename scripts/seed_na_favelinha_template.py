"""Seed the Firestore template registry with the Na Favelinha fixture (S77).
...
Subject replaced (man -> woman, casual at-home) via NBP Pattern B regen.
Uploaded raw (no audio-offset correction at seed time — see
Memory/feedback_kling_audio_lead_and_preview_propagation.md if a lead is
noticed).

Starts as DRAFT — flip to published via `scripts/set_template_status.py`.

Re-running overwrites the Na Favelinha doc.

Usage:
    .venv/bin/python scripts/seed_na_favelinha_template.py
"""
...
log = logging.getLogger("seed_na_favelinha_template")


TEMPLATE_ID = "viral-dances-na-favelinha"
...
NA_FAVELINHA_FIXTURE = {
    "pipeline_class": PIPELINE_MOTION_TRANSFER,
    "outcome": OUTCOME_ONTO_CHARACTER,
    "category": "viral_dances",
    "title": "Na Favelinha",
    "description": "Baile funk, full flow. 🔥",
    "published_status": STATUS_DRAFT,
    "assets": {
        "driving_video_url": "https://assets.speech-2-video.ai/viral-dances/na-favelinha/driving_video.mp4",
        "scene_image_url": None,
        "thumbnail_url": None,
        "preview_video_url": "https://assets.speech-2-video.ai/viral-dances/na-favelinha/preview_video.mp4",
    },
...
def main() -> None:
    ...
    written = upsert_template(TEMPLATE_ID, NA_FAVELINHA_FIXTURE)
    log.info(
