"""V2 Pipeline A Raindrop template — NBP-edit -> Kling Motion Control chain (S82).

Eighth template of the tiktok_dances category. Sister to
scripts/test_high_school_chain.py (Pattern B — holistic regen, DIFFERENT subject).

Source: competitor-app screen recording of a tan-skinned young woman dancing at
NIGHT on a parking-lot sidewalk (streetlamp, parked cars), black sleeveless crop
+ light straight jeans + chunky sneakers, static camera, ~15.0s after a 1s
start-trim. The source scene (night parking lot) overlaps the existing High
School template, so per S82 user direction the regen moves her to a DISTINCT
night scene — a neon-lit city ROOFTOP with a skyline (or a neon-sign alley) —
keeping the night-streetwear vibe, new identity.

Night handling (S80 Like a G6 lesson): the scene is REGEN-framed as a genuine
night photograph lit by neon / city glow — NOT a darkened daylight subject.
...
Usage:
    .venv/bin/python scripts/test_raindrop_chain.py --no-kling
    .venv/bin/python scripts/test_raindrop_chain.py --edited-image ~/Downloads/raindrop_edit_xxx.png --keep-audio
"""
...
DEFAULT_REFERENCE = Path(
    "/Users/saurabhsmacbookair/Downloads/App Templates Prep/Working/raindrop_ref.png"
)

# Pattern B — holistic regen, DIFFERENT subject (S82). Distinct night scene to
# avoid cloning High School: neon-lit city rooftop / skyline (or neon alley),
# night-streetwear. Night regen-framed with motivated neon/city lighting (S80
# Like a G6 lesson). S78 roomy composition + feet-visible. Hybrid-permissive
# menus per feedback_nbp_hybrid_default.md.
RAINDROP_EDIT_PROMPT = (
    "Edit this image:\n"
    "- Replace the subject with a DIFFERENT young woman — similar age "
    "(approximately early-to-mid 20s), slim build, with a confident expression. "
    "She must be clearly a different person from the one in the input (different "
    "facial features, different identity). Different hairstyle from the input "
    "(e.g. a sleek high ponytail, long straight hair, or soft waves — pick one). "
    "Dainty earrings optional. Same general upright standing dance pose, facing "
    "the camera, full body visible from head to feet.\n"
    "- Wardrobe: a night-out streetwear outfit in a similar register to the input "
    "but not identical — e.g. a fitted black sleeveless crop with high-waist "
    "jeans, a cropped tank with wide-leg cargo trousers, or a long-sleeve crop "
    "with baggy jeans — pick one cohesive look, with clean sneakers. NO brand "
    "logos or readable text; any graphic must be a small generic abstract design.\n"
    "- Scene: a nighttime city ROOFTOP — a distant illuminated city skyline, a "
    "warm neon glow and scattered string / neon lights, dark sky above. (A "
    "moody neon-sign alley at night is an acceptable alternative.) Similar "
    "nightlife register to the input but a DISTINCT composition — NOT a parking "
    "lot. CRITICAL: the subject must read as GENUINELY photographed at night, lit "
    "by the warm neon / city glow, exposed consistently with the dark scene. Do "
    "NOT render a brightly daylit subject composited onto a dark background. "
    "Moody nightlife vibe.\n"
    ...
    "ground on BOTH the left and right sides, plus clear ground below the feet — "
    ...
    "ground below them — do NOT crop the feet at the bottom edge. Keep feet "
    ...
    "buttons, no text overlays, no status icons. Just the subject on the rooftop."
)
...
RAINDROP_DRIVING_VIDEO = (
    "https://assets.speech-2-video.ai/viral-dances/raindrop/raw_source.mp4"
)

# Kling config (S73 defaults): v2.6 + pro + video-orientation (driving ~14.8s).
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "raindrop_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False) -> int:
    ...
    key = f"spike-outputs/raindrop-chain/{uuid.uuid4().hex}.png"
    ...
    log.info(
        ...
        RAINDROP_DRIVING_VIDEO, KLING_MODEL_NAME, KLING_CHARACTER_ORIENTATION, KLING_MODE, keep_audio,
    )
    ...
    result = client.generate_and_poll(
        image_url=image_url,
        video_url=RAINDROP_DRIVING_VIDEO,
    ...
    out_path = out_dir / f"raindrop_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument(
        ...
        help=f"Path to Raindrop reference image (default: {DEFAULT_REFERENCE})",
    )
    ap.add_argument(
        "--edited-image",
