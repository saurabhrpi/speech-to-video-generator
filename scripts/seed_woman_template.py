"""Seed the Firestore template registry with the Woman fixture (S80).

Sister to scripts/seed_pole_dance_template.py. Pipeline A
(motion-onto-character via Kling Motion Control), use_nbp_regen=true with the
generic full-body framing hint.

Category: girl_dances. Eighth girl_dances row template.

Preview note (S80): preview generated kling-v2-6 + pro + video orientation.
Subject = different young WHITE woman (Pattern B + S80 row preference), iterated
via NBP edits: matched the source's coral-tank + jeans register, added sofas to
the spacious bright-apartment scene, closer medium framing (feet cropped, per
user). Driver was Topaz 2× upscaled (re-encoded to ~14 Mbps) for sharper face
detail. Kling output 8.0s; END-TRIMMED to 7.0s to drop a consistent abrupt
end-of-clip pose jump (both rolls showed it). Aspect ~0.92:1 (preserved source).

Assets land in the S77 migrated shape directly:
  driving_video_url        -> driving_video.mp4   (trimmed high-bitrate Kling output, runtime driver)
...
Re-running overwrites the Woman doc.

Usage:
    .venv/bin/python scripts/seed_woman_template.py
"""
...
log = logging.getLogger("seed_woman_template")


TEMPLATE_ID = "viral-dances-woman"
...
_BASE = "https://assets.speech-2-video.ai/viral-dances/woman"
...
def main() -> None:
    ...
    written = upsert_template(TEMPLATE_ID, WOMAN_FIXTURE)
    log.info(
