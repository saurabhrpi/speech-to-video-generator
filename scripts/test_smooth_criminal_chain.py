"""V2 Pipeline A Smooth Criminal template — NBP-edit → Kling Motion Control chain (S72).
...
Usage:
    .venv/bin/python scripts/test_smooth_criminal_chain.py
    .venv/bin/python scripts/test_smooth_criminal_chain.py --no-kling
    .venv/bin/python scripts/test_smooth_criminal_chain.py --edited-image ~/Downloads/smooth_criminal_edit_xxx.png
"""
...
DEFAULT_REFERENCE = Path(
    "/Users/saurabhsmacbookair/Downloads/App Templates Prep/Working/Smooth_Criminal_cropped.png"
)

SMOOTH_CRIMINAL_EDIT_PROMPT = (
    "Edit this image:\n"
    "- Remove the dark circular X close-button in the top-left corner. Paint "
    "over it with what would naturally be behind it.\n"
    "- Change the top to a solid beige short-sleeve crop top (same cut as the "
    "original top, no print or graphic, just plain beige). Keep the dark navy "
    "shorts and white sneakers unchanged.\n"
    "- Change the background to an outdoor patio. The patio floor extends "
    "naturally under the subject's feet. Include a small tree on one side and "
    "a soft rolling hill in the background. Warm natural daylight.\n"
    "- Preserve the subject's pose, face, hair, body shape, skin tone, and "
    "identity exactly. She remains a young woman standing in the same mid-dance "
    "pose as the input.\n"
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "smooth_criminal_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False) -> int:
    ...
    key = f"spike-outputs/smooth-criminal-chain/{uuid.uuid4().hex}.png"
    ...
    log.info("Kling submit  driving=%s  keep_audio=%s", SMOOTH_CRIMINAL_DRIVING_VIDEO, keep_audio)
    log.info("Kling prompt:\n%s", GENERIC_KLING_PROMPT)

    t0 = time.time()
    result = client.generate_and_poll(
        image_url=image_url,
        video_url=SMOOTH_CRIMINAL_DRIVING_VIDEO,
    ...
    out_path = out_dir / f"smooth_criminal_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument(
        ...
        help=f"Path to Smooth Criminal reference image (default: {DEFAULT_REFERENCE})",
    )
    ap.add_argument(
        "--edited-image",
