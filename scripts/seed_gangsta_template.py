"""Seed the Firestore template registry with the Gangsta fixture (S67).

Mirrors `scripts/seed_template_registry.py` (which seeded Bombale). Pipeline A
(motion-onto-character via Kling Motion Control), use_nbp_regen=true with the
same generic full-body framing hint we use for Bombale.
...
(`scripts/set_template_hero.py --enable --order 1`).

Re-running overwrites the Gangsta doc.

Usage:
    .venv/bin/python scripts/seed_gangsta_template.py
"""
...
log = logging.getLogger("seed_gangsta_template")


TEMPLATE_ID = "viral-dances-gangsta"

# Generic coherence prompt — identical to Bombale's. The no-overfit policy
# keeps this prompt template free of per-template specifics; framing hints
# (which DO vary) live in `nbp_framing_hint`.
...
GANGSTA_FIXTURE = {
    ...
    "title": "Gangsta",
    "description": "Loose shoulders, steady gaze. No rush. No flex. Just the vibe. 😎",
    "published_status": STATUS_DRAFT,
    "assets": {
        "driving_video_url": "https://assets.speech-2-video.ai/viral-dances/gangsta/driving_video_10s.mp4",
        "scene_image_url": None,
        "thumbnail_url": "https://assets.speech-2-video.ai/viral-dances/gangsta/thumbnail.jpg",
...
def main() -> None:
    ...
    written = upsert_template(TEMPLATE_ID, GANGSTA_FIXTURE)
    log.info(
