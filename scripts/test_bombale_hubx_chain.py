"""V2 Pipeline A Bombale-Hubx template — NBP-edit → Kling Motion Control chain (S73).

NEW APPENDED template (slug: viral-dances-bombale-hubx) — does NOT replace
the existing viral-dances-bombale, ships alongside it. Holistic-regen NBP
pattern (different subject, same mood/wardrobe/scene register).

Kling config: v2.6 + pro + video + ≤15s. Driving video is 11.20s.

Usage:
    .venv/bin/python scripts/test_bombale_hubx_chain.py --no-kling
    .venv/bin/python scripts/test_bombale_hubx_chain.py --edited-image ~/Downloads/bombale_hubx_edit_xxx.jpg --keep-audio
"""
...
DEFAULT_REFERENCE = Path(
    "/Users/saurabhsmacbookair/Downloads/App Templates Prep/bombale_hubx_frame0.png"
)

BOMBALE_HUBX_EDIT_PROMPT = (
    "Edit this image:\n"
    "- Replace the subject with a DIFFERENT young woman — similar age "
    "(approximately late teens / early 20s), athletic slim build, long "
    "flowing dark-brown hair worn loose, with an energetic confident smile. "
    "She must be clearly a different person from the one in the input "
    "(different facial features, different identity), but in the same mood "
    "register. Same general standing dance pose — relaxed stance, arms "
    "loosely bent near her torso, full body visible from head to feet.\n"
    "- Wardrobe: black fitted sleeveless tank top (cropped just above the "
    "navel). Black high-waist skinny pants (smooth fabric, fitted through "
    "the leg, ankle-length). Plain black low-top sneakers.\n"
    "- Scene: a studio with dramatic dual-color gel lighting — a cool "
    "blue/purple wash on the LEFT side of the frame, blending into a warm "
    "red/orange wash on the RIGHT side. Plain neutral light-grey backdrop, "
    "no other props or scene elements. Modern, edgy photoshoot vibe.\n"
    "- Remove ALL UI overlays from the input: the iPhone status bar at top "
    "(time, signal, wifi, battery icons) and the dark circular X close-"
    "button on the left side. Paint over each with what would naturally "
    "be behind it.\n"
    ...
    "feet with comfortable headroom above the hair and floor visible "
    "under her feet."
)
...
BOMBALE_HUBX_DRIVING_VIDEO = (
    "https://assets.speech-2-video.ai/viral-dances/bombale-hubx/driving_video.mp4"
)
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "bombale_hubx_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False) -> int:
    ...
    key = f"spike-outputs/bombale-hubx-chain/{uuid.uuid4().hex}.png"
    ...
    log.info(
        ...
        BOMBALE_HUBX_DRIVING_VIDEO, KLING_MODEL_NAME, KLING_CHARACTER_ORIENTATION, KLING_MODE, keep_audio,
    )
    ...
    result = client.generate_and_poll(
        image_url=image_url,
        video_url=BOMBALE_HUBX_DRIVING_VIDEO,
    ...
    out_path = out_dir / f"bombale_hubx_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument(
        ...
        help=f"Path to Bombale-Hubx reference image (default: {DEFAULT_REFERENCE})",
    )
    ap.add_argument(
        "--edited-image",
