"""Seed the Firestore template registry with the Dale Pa Ve fixture (S82).
...
Preview note (S82): Pattern B holistic regen — full subject + scene swap. Source
driving clip is a man on a beach; regen'd character is a young LATINA woman in a
bright dance / Zumba STUDIO, red long-sleeve crop + baggy wide-leg jeans (S82
user direction). Kling is Outcome-2 so the studio (from the character image) is
the output scene, not the driving video's beach. Preview generated kling-v2-6 +
pro + video orientation; source trimmed from t=1s, then the Kling output was
tail-cropped to 14.5s (end-duplicate mitigation, runbook step 8).

NO audio-lead correction was applied (this output didn't show a noticeable lead),
so driver and preview are simple twins of the tail-cropped master.
...
Assets land in the S77 migrated shape directly:
  ...
  original_driving_video_url -> raw_source.mp4    (revert target — the original beach clip)

After the doc write, the seed generates the first-frame poster (S81): frame0 of
preview_stream.mp4 -> thumbnail.jpg -> assets.thumbnail_url.

Starts as DRAFT — flip to published via `scripts/set_template_status.py`.

Re-running overwrites the Dale Pa Ve doc.

Usage:
    .venv/bin/python scripts/seed_dale_pa_ve_template.py
"""
...
log = logging.getLogger("seed_dale_pa_ve_template")


TEMPLATE_ID = "viral-dances-dale-pa-ve"
...
_BASE = "https://assets.speech-2-video.ai/viral-dances/dale-pa-ve"
...
def main() -> None:
    ...
    written = upsert_template(TEMPLATE_ID, DALE_PA_VE_FIXTURE)
    log.info(
    ...
    # thumbnail.jpg -> assets.thumbnail_url. Requires preview_stream.mp4 on R2.
    try:
