"""V2 Pipeline A Pinky Up template — NBP-edit → Kling Motion Control chain (S81).

Second template of the tiktok_dances category. Sister to
scripts/test_lets_go_chain.py (Pattern B — holistic regen with a DIFFERENT
subject; source is an identifiable real creator).

Source: competitor-app screen recording of a light-skinned young woman dancing
against a plain white wall in black athletic wear (Adidas/Nike logos visible),
static camera, ~15.2s after a 1s start-trim. Per S81 user direction: keep a
light-skinned woman (new identity), move to a DIFFERENT light-colored wall +
fresh athletic wear, brand logos stripped. S78 roomy composition (static camera,
wide leg stances → lateral room).
...
Usage:
    .venv/bin/python scripts/test_pinky_up_chain.py --no-kling
    .venv/bin/python scripts/test_pinky_up_chain.py --edited-image ~/Downloads/pinky_up_edit_xxx.png --keep-audio
"""
...
DEFAULT_REFERENCE = Path(
    "/Users/saurabhsmacbookair/Downloads/App Templates Prep/Working/pinky_up_ref.png"
)

# Pattern B — holistic regen, DIFFERENT subject. S81 direction: keep a
# light-skinned woman (new identity), DIFFERENT light-colored wall + fresh
# athletic wear, strip the source's Adidas/Nike logos. Hybrid-permissive menus
# (register not specifics) per Memory/feedback_nbp_hybrid_default.md. S78 roomy
# composition (static-camera source, wide stances → lateral room; no dup risk).
PINKY_UP_EDIT_PROMPT = (
    "Edit this image:\n"
    "- Replace the subject with a DIFFERENT young woman — fair-to-medium "
    "skin tone, similar age (approximately late teens / early 20s), athletic "
    "toned build, with a confident energetic expression. She must be clearly "
    "a different person from the one in the input (different facial features, "
    "different identity). Different hairstyle from the input (e.g. a high "
    "ponytail, loose waves, or a half-up style — pick one). Same general "
    "upright standing dance pose, facing the camera, full body visible from "
    "head to feet.\n"
    "- Wardrobe: athletic streetwear that is NOT the input's all-black outfit. "
    "E.g. a pastel cropped hoodie with matching shorts, a white tee with "
    "colored bike shorts, or a light two-piece track set — pick one cohesive "
    "athletic outfit. Clean low-top sneakers with crew socks. ABSOLUTELY NO "
    "brand logos or readable text — the input's Adidas three-stripes and Nike "
    "swoosh must NOT appear; any graphic must be a small generic abstract "
    "design only.\n"
    "- Scene: a clean minimal studio-like setting against a smooth painted "
    "wall in a DIFFERENT light color from the input's plain white — e.g. soft "
    "sage green, pale warm blush, light dusty blue, or warm cream (pick one). "
    "Light-toned floor under her feet. Bright soft daylight, clean minimal vibe.\n"
    ...
    "roughly EQUAL open wall on BOTH the left and right sides, plus clear "
    "floor below the feet — room for lateral dance movement and a full arm "
    ...
    "floor below them — do NOT crop the feet/ankles at the bottom edge. Keep "
    ...
    "buttons, no text overlays, no status icons. Just the subject by the wall."
)
...
PINKY_UP_DRIVING_VIDEO = (
    "https://assets.speech-2-video.ai/viral-dances/pinky-up/raw_source.mp4"
)

# Kling config (S73 defaults): v2.6 + pro + video-orientation (driving ~15.2s).
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "pinky_up_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False) -> int:
    ...
    key = f"spike-outputs/pinky-up-chain/{uuid.uuid4().hex}.png"
    ...
    log.info(
        ...
        PINKY_UP_DRIVING_VIDEO, KLING_MODEL_NAME, KLING_CHARACTER_ORIENTATION, KLING_MODE, keep_audio,
    )
    ...
    result = client.generate_and_poll(
        image_url=image_url,
        video_url=PINKY_UP_DRIVING_VIDEO,
    ...
    out_path = out_dir / f"pinky_up_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument(
        ...
        help=f"Path to Pinky Up reference image (default: {DEFAULT_REFERENCE})",
    )
    ap.add_argument(
        "--edited-image",
