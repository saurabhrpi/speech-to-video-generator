"""Seed the Firestore template registry with the Dance Flow fixture (S77).
...
Subject = similar-looking man (Pattern B), scene changed to a European
seaside-hotel balcony with a beach background. Uploaded raw.

Starts as DRAFT — flip to published via `scripts/set_template_status.py`.

Re-running overwrites the Dance Flow doc.

Usage:
    .venv/bin/python scripts/seed_dance_flow_template.py
"""
...
log = logging.getLogger("seed_dance_flow_template")


TEMPLATE_ID = "viral-dances-dance-flow"
...
DANCE_FLOW_FIXTURE = {
    ...
    "title": "Dance Flow",
    "description": "Golden-hour groove. 🌅",
    "published_status": STATUS_DRAFT,
    "assets": {
        "driving_video_url": "https://assets.speech-2-video.ai/viral-dances/dance-flow/driving_video.mp4",
        "scene_image_url": None,
        "thumbnail_url": None,
        "preview_video_url": "https://assets.speech-2-video.ai/viral-dances/dance-flow/preview_video.mp4",
    },
...
def main() -> None:
    ...
    written = upsert_template(TEMPLATE_ID, DANCE_FLOW_FIXTURE)
    log.info(
