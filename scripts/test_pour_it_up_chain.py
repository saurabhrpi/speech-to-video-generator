"""V2 Pipeline A Pour It Up template — NBP-edit → Kling Motion Control chain (S79).

Sixth template of the "Girl Dances" row (category girl_dances).
Sister to scripts/test_give_it_up_chain.py.

Pour It Up is an UPRIGHT, camera-facing street/night dance (dynamic full-body
arm gestures) shot outdoors in a parking lot at night — Kling-trackable. Uses
the runbook's S78 standing composition: roomy/wider full-body, centered,
feet+floor visible, generous lateral room.

Pattern B (different woman) — the source is an identifiable competitor-app
dancer; we swap in a genuinely different subject. Styling (user decision S79):
match the source's wardrobe STYLE AND COVERAGE exactly — fitted tank top + slim
jeans — neither more conservative nor more revealing; just a different woman +
different colors. Keep the moody outdoor-night register.

Source: Pour_It_Up_start_at_0.5_sec.mov (1284×1400, ~1:1, ~14.2s; trim the first
0.5s for the driver). Reference frame = clip t=2s (pour_it_up_frame_t2.png).

Usage:
    .venv/bin/python scripts/test_pour_it_up_chain.py --no-kling
    .venv/bin/python scripts/test_pour_it_up_chain.py --edited-image ~/Downloads/pour_it_up_edit_xxx.png --keep-audio
"""
...
DEFAULT_REFERENCE = Path(
    "/Users/saurabhsmacbookair/Downloads/App Templates Prep/Working/Girl Dances/pour_it_up_frame_t2.png"
)

POUR_IT_UP_EDIT_PROMPT = (
    "Edit this image:\n"
    ...
    "hair.\n"
    "- Wardrobe: MATCH the input's style and coverage EXACTLY — a fitted tank "
    "top with slim jeans and ankle boots, same cut and same skin coverage as "
    "the input (do NOT make it more conservative, and do NOT make it more "
    "revealing — match it). Just change the COLORS: a tank in a color that is "
    "NOT the input's black (e.g. white, deep red, emerald, or lilac — pick "
    "one), and jeans in a different wash. No readable text or logos.\n"
    "- Scene: match the input's moody OUTDOOR NIGHT register but make it a "
    "SPACIOUS open setting — a wide empty street, plaza, or open lot at night "
    "with warm ambient streetlights and a clear evening sky, and a WIDE "
    "expanse of open ground extending well to her LEFT AND RIGHT. Nothing "
    "within arm's reach on either side. Keep the SUBJECT clearly lit and "
    "well-exposed against the night background (her body and face must read "
    "clearly).\n"
    ...
    "GENEROUS, roughly EQUAL open empty ground on BOTH the left and right "
    "sides, plus clear ground below the feet — room for lateral dance movement "
    "in either direction.\n"
    "- FEET AND FLOOR (critical): feet FULLY visible with a clear margin of "
    "ground below them — do NOT crop the feet/ankles at the bottom edge. Keep "
...
POUR_IT_UP_DRIVING_VIDEO = (
    "https://assets.speech-2-video.ai/viral-dances/pour-it-up/raw_source.mp4"
)
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "pour_it_up_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False,
              driving_video: str = POUR_IT_UP_DRIVING_VIDEO) -> int:
    settings = get_settings()
    selfies_bucket = settings.r2_selfies_bucket
    key = f"spike-outputs/pour-it-up-chain/{uuid.uuid4().hex}.png"
    ...
    out_path = out_dir / f"pour_it_up_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument("--driving-video", default=POUR_IT_UP_DRIVING_VIDEO,
