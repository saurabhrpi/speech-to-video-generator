"""Seed the Firestore template registry with the Soda Pop Energy fixture (S82).
...
A distinct "Soda Pop" variant from viral-dances-soda-pop-baby-dance.

Category: tiktok_dances (mobile label override -> "TikTok Dances").

Preview note (S82): Pattern B holistic regen — DIFFERENT young man in a daytime
COUNTRYSIDE field (rolling hills, trees), cream henley + dark olive jeans + boots
(S82 user direction: keep it a man, countryside field). Full-body framing pushed
since the source was a tight medium shot; source trimmed from t=0.5s.
...
Re-running overwrites the Soda Pop Energy doc.

Usage:
    .venv/bin/python scripts/seed_soda_pop_energy_template.py
"""
...
log = logging.getLogger("seed_soda_pop_energy_template")


TEMPLATE_ID = "viral-dances-soda-pop-energy"
...
_BASE = "https://assets.speech-2-video.ai/viral-dances/soda-pop-energy"

SODA_POP_ENERGY_FIXTURE = {
    "pipeline_class": PIPELINE_MOTION_TRANSFER,
    "outcome": OUTCOME_ONTO_CHARACTER,
    "category": "tiktok_dances",
    "title": "Soda Pop Energy",
    "description": "Soda pop energy. 🌾⚡",
...
def main() -> None:
    ...
    written = upsert_template(TEMPLATE_ID, SODA_POP_ENERGY_FIXTURE)
    log.info(
