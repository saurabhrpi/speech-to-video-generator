"""V2 Pipeline A Baby Dance (Hubx redo) — NBP-edit → Kling Motion Control chain (S73).

REPLACES the existing viral-dances-baby-dance preview with a longer (9.61s)
version. Same Firestore template_id, same R2 URL — just new file. Holistic-
regen NBP pattern (different toddler subject, same mood/wardrobe/scene register).

Per existing template convention, the preview features a CHILD stand-in;
runtime applies adult selfie via Kling (acceptable per memory
project_gallery_crash_resilience.md / S70 ToDo note).

Kling config: v2.6 + pro + video + ≤15s. Driving video is 9.61s.

Usage:
    .venv/bin/python scripts/test_baby_dance_hubx_chain.py --no-kling
    .venv/bin/python scripts/test_baby_dance_hubx_chain.py --edited-image ~/Downloads/baby_dance_hubx_edit_xxx.jpg --keep-audio
"""
...
DEFAULT_REFERENCE = Path(
    "/Users/saurabhsmacbookair/Downloads/App Templates Prep/baby_dance_hubx_frame0.png"
)

BABY_DANCE_HUBX_EDIT_PROMPT = (
    "Edit this image:\n"
    "- Replace the subject with a DIFFERENT young child — similar age "
    "(approximately 2-3 years old), with a different hair color and "
    "different facial features from the child in the input (e.g. light "
    "brown / chestnut hair instead of blonde, or a different hair style "
    "with bangs / curls). Same playful energetic mood, same dynamic mid-"
    "dance pose with arms gesturing, full body visible from head to feet.\n"
    "- Wardrobe: cozy casual toddler outfit — a soft pastel-colored crewneck "
    "long-sleeve top (e.g. mint, peach, lavender, or buttercream — pick a "
    "register that's not the input's pink) and matching soft jogger-style "
    "pants. Small white or off-white sneakers.\n"
    "- Scene: a cozy indoor home — soft natural daylight from a window, "
    "a comfortable living room or play room. Include warm wood furniture "
    "or soft furnishings in the background, a few subtle toys visible "
    "around the edges, light-toned walls. Lived-in, intimate domestic "
    "vibe — similar register to the input but distinct in composition.\n"
    ...
    "with comfortable headroom above the hair and floor visible under the "
    "child's feet."
)
...
BABY_DANCE_HUBX_DRIVING_VIDEO = (
    "https://assets.speech-2-video.ai/viral-dances/baby-dance/driving_video.mp4"
)
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "baby_dance_hubx_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False) -> int:
    ...
    key = f"spike-outputs/baby-dance-hubx-chain/{uuid.uuid4().hex}.png"
    ...
    log.info(
        ...
        BABY_DANCE_HUBX_DRIVING_VIDEO, KLING_MODEL_NAME, KLING_CHARACTER_ORIENTATION, KLING_MODE, keep_audio,
    )
    ...
    result = client.generate_and_poll(
        image_url=image_url,
        video_url=BABY_DANCE_HUBX_DRIVING_VIDEO,
    ...
    out_path = out_dir / f"baby_dance_hubx_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument(
        ...
        help=f"Path to Baby Dance Hubx reference image (default: {DEFAULT_REFERENCE})",
    )
    ap.add_argument(
        "--edited-image",
