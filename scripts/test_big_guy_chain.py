"""V2 Pipeline A Big Guy template — NBP-edit -> Kling Motion Control chain (S82).

Ninth template of the tiktok_dances category. Sister to
scripts/test_lush_life_chain.py (Pattern B — holistic regen, DIFFERENT subject).
"Big Guy" is the SONG/trend name; the source subject is a woman.

Source: competitor-app screen recording of a light-skinned young woman dancing in
a grassy MEADOW / field with a distant mountain backdrop (daytime, soft overcast
light), white long-sleeve crop + mustard suede mini skirt, static camera, ~13.0s
after a 0.5s start-trim. The source's tall grass hides the feet — the regen pushes
shorter grass / a clearing so the feet are visible for full-body framing.

S82 user direction: keep the daytime meadow/mountain scene + boho-casual register,
new light-skinned identity. Daytime scene (no night-relight needed).
...
Usage:
    .venv/bin/python scripts/test_big_guy_chain.py --no-kling
    .venv/bin/python scripts/test_big_guy_chain.py --edited-image ~/Downloads/big_guy_edit_xxx.png --keep-audio
"""
...
DEFAULT_REFERENCE = Path(
    "/Users/saurabhsmacbookair/Downloads/App Templates Prep/Working/big_guy_ref.png"
)

# Pattern B — holistic regen, DIFFERENT subject (S82). Keep the daytime meadow /
# mountain scene + boho-casual register, new light-skinned woman. Daytime soft
# light (no night-relight). Source's tall grass hides the feet -> push shorter
# grass / clearing so feet are visible. Hybrid-permissive menus per
# feedback_nbp_hybrid_default.md.
BIG_GUY_EDIT_PROMPT = (
    "Edit this image:\n"
    ...
    "warm joyful expression. She must be clearly a different person from the one "
    "in the input (different facial features, different identity). Different "
    "hairstyle from the input (e.g. long straight hair, soft waves, or a loose "
    "braid — pick one). Same general upright standing dance pose, facing the "
    "camera, full body visible from head to feet.\n"
    "- Wardrobe: a boho-casual daytime outfit in a similar register to the input "
    "but not identical — e.g. a white long-sleeve crop top with a tan / mustard "
    "suede mini skirt, a cream knit crop with a denim mini, or a fitted light "
    "blouse with an earth-tone skirt — pick one cohesive look. NO brand logos or "
    "readable text; any graphic must be a small generic abstract design.\n"
    "- Scene: a sunlit grassy MEADOW / open field with a distant mountain "
    "backdrop and soft daytime sky. Similar register to the input but a distinct "
    "composition. Genuine soft daytime outdoor lighting on the subject. Calm "
    "summer-countryside vibe.\n"
    ...
    "field on BOTH the left and right sides, plus clear ground below the feet — "
    "room for lateral dance movement and a full arm wingspan in either direction. "
    "Nothing within arm's reach on either side.\n"
    "- FEET AND FLOOR (critical): the input's tall grass hides the feet — instead "
    "place her in SHORTER grass or a grassy clearing so the feet are FULLY "
    "visible with a clear margin of ground below them. Do NOT crop or hide the "
    "feet at the bottom edge. Keep feet >=8-10% of frame height above the bottom.\n"
...
BIG_GUY_DRIVING_VIDEO = (
    "https://assets.speech-2-video.ai/viral-dances/big-guy/raw_source.mp4"
)

# Kling config (S73 defaults): v2.6 + pro + video-orientation (driving ~12.9s).
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "big_guy_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False) -> int:
    ...
    key = f"spike-outputs/big-guy-chain/{uuid.uuid4().hex}.png"
    ...
    log.info(
        ...
        BIG_GUY_DRIVING_VIDEO, KLING_MODEL_NAME, KLING_CHARACTER_ORIENTATION, KLING_MODE, keep_audio,
    )
    ...
    result = client.generate_and_poll(
        image_url=image_url,
        video_url=BIG_GUY_DRIVING_VIDEO,
    ...
    out_path = out_dir / f"big_guy_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument(
        ...
        help=f"Path to Big Guy reference image (default: {DEFAULT_REFERENCE})",
    )
    ap.add_argument(
        "--edited-image",
