"""V2 Pipeline A Freeze template — NBP-edit -> Kling Motion Control chain (S82).
...
Source: competitor-app screen recording of a light-skinned young woman dancing at
NIGHT in a warm upscale restaurant/cafe interior (pendant lighting, tables,
potted plants, terrazzo floor), black strapless top + tan tweed mini skirt,
static-ish camera, ~14.2s full duration. Per S82 user direction ("just use it as
it is"): NO trim, and keep the source's own register — a new light-skinned woman
identity in the same night-restaurant going-out scene.

Night handling (S80 Like a G6 lesson): the scene is REGEN-framed as a genuine
warm indoor night photograph with motivated lighting — NOT a darkened daylight
subject.
...
Usage:
    .venv/bin/python scripts/test_freeze_chain.py --no-kling
    .venv/bin/python scripts/test_freeze_chain.py --edited-image ~/Downloads/freeze_edit_xxx.png --keep-audio
"""
...
DEFAULT_REFERENCE = Path(
    "/Users/saurabhsmacbookair/Downloads/App Templates Prep/Working/freeze_ref.png"
)

# Pattern B — holistic regen, DIFFERENT subject. S82 direction: keep a
# light-skinned woman (new identity), keep the NIGHT restaurant/cafe + going-out
# fit. Night is regen-framed with motivated warm interior lighting (S80 Like a G6
# lesson — a darken edit leaves the subject daylit). S78 roomy composition +
# explicit feet-visible. Hybrid-permissive menus per feedback_nbp_hybrid_default.md.
FREEZE_EDIT_PROMPT = (
    "Edit this image:\n"
    ...
    "confident expression. She must be clearly a different person from the one "
    "in the input (different facial features, different identity). Different "
    "hairstyle from the input (e.g. sleek straight, soft loose waves, or a low "
    "bun — pick one). Dainty hoop earrings optional. Same general upright "
    "standing dance pose, facing the camera, full body visible from head to feet.\n"
    "- Wardrobe: a night-out outfit in a similar register to the input but not "
    "identical — e.g. a black strapless bandeau top with a LOOSE flowy A-line "
    "mini skirt, a fitted dark satin top with a lightly pleated mini skirt, or a "
    "sleek dark top with a relaxed flared mini — pick one cohesive going-out "
    "look. The skirt must be a LOOSER mini that flares gently away from the body "
    "(NOT a tight bodycon / pencil mini). Optional dainty layered necklaces. NO "
    "brand logos or readable text; any graphic must be a small generic abstract design.\n"
    "- Scene: a warm upscale restaurant / cafe interior at night — soft pendant "
    "lighting, tables and chairs, leafy potted plants, a polished terrazzo or "
    "tiled floor. Similar register to the input but a distinct composition. "
    "CRITICAL: the subject must read as GENUINELY photographed indoors at night "
    "under warm ambient restaurant light, exposed consistently with the cozy "
    "interior. Do NOT render a brightly daylit subject composited onto a dark "
    "background. Moody nightlife vibe.\n"
    ...
    "floor on BOTH the left and right sides, plus clear floor below the feet — "
    ...
    "buttons, no text overlays, no status icons. Just the subject in the restaurant."
)
...
FREEZE_DRIVING_VIDEO = (
    "https://assets.speech-2-video.ai/viral-dances/freeze/raw_source.mp4"
)

# Kling config (S73 defaults): v2.6 + pro + video-orientation (driving ~14.2s).
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "freeze_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False) -> int:
    ...
    key = f"spike-outputs/freeze-chain/{uuid.uuid4().hex}.png"
    ...
    log.info(
        ...
        FREEZE_DRIVING_VIDEO, KLING_MODEL_NAME, KLING_CHARACTER_ORIENTATION, KLING_MODE, keep_audio,
    )
    ...
    result = client.generate_and_poll(
        image_url=image_url,
        video_url=FREEZE_DRIVING_VIDEO,
    ...
    out_path = out_dir / f"freeze_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument(
        ...
        help=f"Path to Freeze reference image (default: {DEFAULT_REFERENCE})",
    )
    ap.add_argument(
        "--edited-image",
