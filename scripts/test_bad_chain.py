"""V2 Pipeline A Bad template — NBP-edit → Kling Motion Control chain (S71).

Sister to scripts/test_beat_it_chain.py. Same shape; only the per-template
...
Usage:
    .venv/bin/python scripts/test_bad_chain.py
    .venv/bin/python scripts/test_bad_chain.py --no-kling
    .venv/bin/python scripts/test_bad_chain.py --edited-image ~/Downloads/bad_edit_xxx.jpg
    .venv/bin/python scripts/test_bad_chain.py --edited-image ~/Downloads/bad_edit_xxx.jpg --keep-audio
"""
...
DEFAULT_REFERENCE = Path(
    "/Users/saurabhsmacbookair/Downloads/App Templates Prep/Working/Bad_cropped.png"
)

BAD_EDIT_PROMPT = (
    "Edit this image:\n"
    "- Remove the dark circular X close-button in the top-left corner. Paint "
    "over it with what would naturally be behind it (sky / stone column).\n"
    "- REPLACE the male dancer entirely with a WHITE WOMAN in her EARLY 20s. "
    "Pale fair skin, no facial hair, long natural hair falling past the "
    "shoulders, slim athletic build, youthful feminine features. PRESERVE "
    "THE DANCE POSE PRECISELY — same hand positions, same foot positions, "
    "same body angle and momentum as the original subject.\n"
    "- Outfit: a SLEEVELESS BLUE DENIM jacket / vest (medium-wash denim, "
    "cropped armholes — no sleeves), worn open over a BLACK fitted BANDEAU "
    "top (clearly visible across the chest under the open denim vest), "
    "MEDIUM-WASH BLUE DENIM jeans (visible blue denim texture, not too "
    "dark), and DARK BLUE low-top sneakers.\n"
    "- Change the background to an OPEN European city avenue in crisp "
    "daylight: a wide, busy street with cars and traffic visible in the far "
    "distance, completely open surroundings, big sky. Directly behind the "
    "dancer rises a MASSIVE classical European-style stone column / pillar "
    "(Trafalgar / Brandenburg / Vendôme scale — fluted classical stone, "
    "weathered tan-grey), clearly part of a much larger monument that "
    "extends UP and OUT past the top of the frame. The monument MUST NOT be "
    "fully in frame — only its base and the lower portion of the column "
    "shaft are visible, with the rest implied above. The asphalt / paving of "
    "the street must be clearly visible under the dancer's feet so the "
    "ground plane reads cleanly.\n"
    "- Output a clean photographic frame with no UI elements, no buttons, no "
    "text, and no watermarks. Natural editorial lighting, polished register, "
    "full-body framing head to feet, with the street visible beneath the "
    "dancer."
)
...
BAD_DRIVING_VIDEO = (
    "https://assets.speech-2-video.ai/viral-dances/bad/driving_video.mp4"
)
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "bad_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False) -> int:
    ...
    ext = Path(edited_image_path).suffix.lstrip(".") or "png"
    key = f"spike-outputs/bad-chain/{uuid.uuid4().hex}.{ext}"
    ...
    log.info("Kling submit  driving=%s  keep_audio=%s", BAD_DRIVING_VIDEO, keep_audio)
    log.info("Kling prompt:\n%s", GENERIC_KLING_PROMPT)

    t0 = time.time()
    result = client.generate_and_poll(
        image_url=image_url,
        video_url=BAD_DRIVING_VIDEO,
    ...
    out_path = out_dir / f"bad_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument(
        "--keep-audio",
        action="store_true",
        help="Pass keep_original_sound=yes to Kling (dance templates should pass this — see runbook)",
    )
    ap.add_argument(
        "--out-dir",
