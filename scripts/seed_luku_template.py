"""Seed the Firestore template registry with the Luku fixture (S83).

Sixteenth template of the tiktok_dances category. Sister to
...
Preview note (S83): Pattern B holistic regen — DIFFERENT older white man,
salt-and-pepper hair, 50s-60s, fit dad-bod (preserves the comedic-dad register
of the source), in a clean SUBURBAN BACKYARD daytime. Source had a stainless-
steel BBQ grill flanking the subject; the regen relocated him to an open lawn
with the fence pushed far back and no patio clutter.

Wardrobe = Hawaiian / camp shirt (subtle palm-leaf pattern) + khaki chinos +
boat shoes (source = solid dark navy polo + dark slim jeans). Source trimmed
to 15s from a 0.5s start (the first 0.5s of the source mov is the competitor-
app home screen).

⚠️ LATERAL ROOM CLAUSE (S83 — per
Memory/reference_kling_mc_aspect_inherits_nbp.md): the chain prompt baked in
the explicit "55-65% frame height, generous open lawn on both sides, nothing
within arm's reach" clause from the start.

AUDIO-LEAD: not applied this template — user verified the roll's audio was
already in sync (the Kling MC v2.6+pro+video audio-lead is per-gen-variable
per Memory/feedback_kling_audio_lead_and_preview_propagation.md). Driver = raw
...
Re-running overwrites the Luku doc.

Usage:
    .venv/bin/python scripts/seed_luku_template.py
"""
...
log = logging.getLogger("seed_luku_template")


TEMPLATE_ID = "viral-dances-luku"
...
_BASE = "https://assets.speech-2-video.ai/viral-dances/luku"
...
LUKU_FIXTURE = {
    ...
    "title": "Luku",
    "description": "Dad-mode unlocked. 🌿☀️",
...
def main() -> None:
    ...
    written = upsert_template(TEMPLATE_ID, LUKU_FIXTURE)
    log.info(
