"""V2 Pipeline A Bad-Chef template — NBP-edit → Kling Motion Control chain (S75).

Sister to scripts/test_rasputin_chain.py and scripts/test_no_batidao_chain.py.

Variant of `viral-dances-bad`: same source MP4 (Bad.mov, source dancer is
already chef-themed in front of a food truck), retransposed into an indoor
restaurant kitchen.

Pattern B (S73 hybrid-permissive): holistic regen with a DIFFERENT subject.
User direction (S75): similar looks + mood, chef wearing an apron in a
restaurant kitchen.

The Kling prompt stays the generic coherence prompt (no-overfit policy
per Memory/feedback_no_overfit_prompts.md).

Note: source frame is mid-body, dancing; preview targets full body for
parity with runtime regen output. Composition lock is loosened
(source-vs-output framing differs by design); kept explicit "single subject
only" guard against Kling duplicate-subject hallucination.

Usage:
    .venv/bin/python scripts/test_bad_chef_chain.py
    .venv/bin/python scripts/test_bad_chef_chain.py --no-kling
    .venv/bin/python scripts/test_bad_chef_chain.py --edited-image ~/Downloads/bad_chef_edit_xxx.png
"""
...
BAD_CHEF_EDIT_PROMPT = (
    ...
    "input's short dark crop (e.g. chestnut-brown side-parted style, "
    "dark wavy medium-length swept back, or a short fade with a clean "
    "edge-up — pick one). May have a short trimmed beard or be clean-"
    "shaven (pick one — different from the input). Same energetic, "
    "playful, confident expression — mid-dance flow, lively. Same "
    "general standing dance pose as the input — low athletic stance, "
    "arms expressive and swinging out from the body, knees slightly "
    "bent, full body visible from head to feet.\n"
    "- Wardrobe: professional chef register. A clean white short-sleeve "
    "cotton tee OR a white chef's jacket (pick one). A bib apron in a "
    "tone that is NOT the input's brown leather — e.g. navy denim, "
    "charcoal canvas, or classic white with crossed straps (pick one). "
    "Dark trousers — e.g. black slim chinos, charcoal kitchen trousers, "
    "or dark indigo denim (pick one). Sturdy plain low-profile shoes — "
    "black leather slip-ons, white canvas sneakers, or dark cross-"
    "trainers (pick one).\n"
    "- Scene: a LARGE, SPACIOUS commercial / catering / hotel kitchen "
    "— much bigger than a typical restaurant kitchen. Think industrial "
    "culinary school, banquet kitchen, or large catering operation. "
    "The subject stands in the MIDDLE of a WIDE, OPEN floor expanse — "
    "wide-open clear floor extending well to the LEFT AND RIGHT of "
    "the subject (generous lateral floor space, at least the subject's "
    "full body-width clear on each side). Stainless-steel prep "
    "counters, gas ranges, hanging cookware racks, and prep stations "
    "are visible in the background and along the far periphery, but "
    "they MUST NOT crowd the subject or be within arm's reach — keep "
    "all equipment well behind / well to the sides. Polished concrete "
    "or large-tile floor underfoot, gently reflecting overhead lights. "
    "Warm overhead lighting mixed with cooler ambient fill, lived-in "
    "and active atmosphere — not pristine showroom.\n"
    "- Remove ALL UI overlays from the input: the dark circular X "
    "close-button in the top-left, the iPhone status-bar elements at "
    "the top (the time '7:34' at top-left-center and the signal / wifi "
    ...
    "duplicate, or mirror image. Just one chef standing in the kitchen.\n"
    ...
    "subject will moonwalk laterally across this floor in the next "
    "step — the frame needs ROOM on each side for that motion). The "
    "face MUST be clearly visible (do not crop the head or chin)."
)
...
# ~/Downloads/template_assets --template viral-dances-bad-chef --no-update-registry`
# (with the trimmed Bad clip staged at
#  ~/Downloads/template_assets/viral-dances/bad-chef/driving_video.mp4)
BAD_CHEF_DRIVING_VIDEO = (
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "bad_chef_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False) -> int:
    ...
    key = f"spike-outputs/bad-chef-chain/{uuid.uuid4().hex}.png"
    ...
    log.info(
        ...
        BAD_CHEF_DRIVING_VIDEO, KLING_MODEL_NAME, KLING_CHARACTER_ORIENTATION, KLING_MODE, keep_audio,
    )
    ...
    result = client.generate_and_poll(
        image_url=image_url,
        video_url=BAD_CHEF_DRIVING_VIDEO,
    ...
    out_path = out_dir / f"bad_chef_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument(
        ...
        help=f"Path to Bad-Chef reference image (default: {DEFAULT_REFERENCE})",
    )
    ap.add_argument(
        "--edited-image",
