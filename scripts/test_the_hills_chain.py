...
THE_HILLS_EDIT_PROMPT = (
    ...
    "- Wardrobe: a casual, tasteful at-home dance outfit that is NOT the "
    "input's all-black cami and leggings. E.g. a fitted pastel crop top with "
    "high-waist bike shorts, a relaxed cropped tee with soft joggers, a "
    "ribbed tank with denim shorts, or a light athleisure set — pick one, in "
    "cheerful non-black colors. Modest, tasteful coverage. No readable text "
    "or logos.\n"
    "- Scene: INDOORS at home — a bright, cozy domestic interior (a warm "
    "living room or a tidy bedroom) with soft neutral walls, natural daylight "
    "from a window, and clear floor space extending to the LEFT AND RIGHT. "
    "Distinct from the input's plain studio wall. No furniture within arm's "
    "reach. CRITICAL: do NOT include mirrors or any reflective surfaces that "
    "could show a reflection of the subject.\n"
    ...
    "the input — same head position, same body size relative to the frame "
    "edges, same camera-to-subject distance. Do NOT zoom in, do NOT zoom out. "
    "Preserve the input's framing exactly. Ensure the raised arms are not "
    "clipped at the side edges.\n"
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False) -> int:
    ...
    log.info("Kling submit  driving=%s  model=%s  orientation=%s  mode=%s  keep_audio=%s",
        THE_HILLS_DRIVING_VIDEO, KLING_MODEL_NAME, KLING_CHARACTER_ORIENTATION, KLING_MODE, keep_audio)
    log.info("Kling prompt:\n%s", GENERIC_KLING_PROMPT)

    t0 = time.time()
    result = client.generate_and_poll(
        image_url=image_url, video_url=THE_HILLS_DRIVING_VIDEO,
