"""Seed the Firestore template registry with the Give It Up fixture (S79).

Sister to scripts/seed_telephone_template.py. Pipeline A (motion-onto-character
via Kling Motion Control), use_nbp_regen=true with the generic full-body
framing hint.

Category: girl_dances. Fifth girl_dances row template.

Preview note (S79): preview generated kling-v2-6 + pro + video orientation,
driven off a Topaz-2k-upscaled driver (sharper hands; native fps). Subject =
different young woman (Pattern B), cream ribbed top + light-blue jeans + boots,
spacious sunlit room (S78 roomy composition). 14.8s, no end-trim (clean tail).
A fast hands-joined→gesture transition reads slightly jumpy (Kling interpolation
limit; v3 trial did not fix it — accepted). No audio-lead correction needed.
...
Re-running overwrites the Give It Up doc.

Usage:
    .venv/bin/python scripts/seed_give_it_up_template.py
"""
...
log = logging.getLogger("seed_give_it_up_template")


TEMPLATE_ID = "viral-dances-give-it-up"
...
_BASE = "https://assets.speech-2-video.ai/viral-dances/give-it-up"
...
def main() -> None:
    ...
    written = upsert_template(TEMPLATE_ID, GIVE_IT_UP_FIXTURE)
    log.info(
