"""V2 Pipeline A Give It Up template — NBP-edit → Kling Motion Control chain (S79).

Fifth template of the "Girl Dances" row (category girl_dances).
Sister to scripts/test_telephone_chain.py.

Give It Up (to Me) is an UPRIGHT, camera-facing standing dance (full-body, with
a back-to-camera turn mid-clip) shot against a plain neutral backdrop —
Kling-trackable. Uses the runbook's S78 standing composition: roomy/wider
full-body, centered, feet+floor visible, generous lateral room.
...
modest casual register (fitted top + slim jeans, plain studio scene) but in a
clearly different color + a more spacious interior.

Source: Give_it_Up_to_Me.mov (1284×1384, ~1:1, ~15.3s; trim the first 0.5s for
the driver). Reference frame = clip t=2s (give_it_up_frame_t2.png) — clear
front-facing face (t=7 is a back-to-camera turn).

Usage:
    .venv/bin/python scripts/test_give_it_up_chain.py --no-kling
    .venv/bin/python scripts/test_give_it_up_chain.py --edited-image ~/Downloads/give_it_up_edit_xxx.png --keep-audio
"""
...
DEFAULT_REFERENCE = Path(
    "/Users/saurabhsmacbookair/Downloads/App Templates Prep/Working/Girl Dances/give_it_up_frame_t2.png"
)

GIVE_IT_UP_EDIT_PROMPT = (
    "Edit this image:\n"
    ...
    "hair.\n"
    "- Wardrobe: match the input's modest casual register but in clearly "
    "DIFFERENT colors — a fitted top (e.g. a ribbed long-sleeve, knit top, or "
    "fitted blouse) in a color that is NOT the input's black (e.g. cream, "
    "dusty rose, sage, or terracotta — pick one), with slim jeans or trousers "
    "in a DIFFERENT wash from the input's charcoal grey (e.g. light blue, "
    "white, or tan), and boots or heels. Keep the same modest coverage as the "
    "input. No readable text or logos.\n"
    "- Scene: match the input's clean, plain register but make it a SPACIOUS, "
    "airy interior — a large, uncluttered, sunlit room (e.g. a bright "
    "minimalist studio, a sunlit loft, or an open neutral room), with soft "
    "natural daylight and a WIDE expanse of clear, open, uncluttered floor "
    "extending well to her LEFT AND RIGHT. Nothing within arm's reach on "
    "either side.\n"
...
GIVE_IT_UP_DRIVING_VIDEO = (
    "https://assets.speech-2-video.ai/viral-dances/give-it-up/raw_source.mp4"
)
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "give_it_up_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False,
              driving_video: str = GIVE_IT_UP_DRIVING_VIDEO) -> int:
    settings = get_settings()
    selfies_bucket = settings.r2_selfies_bucket
    key = f"spike-outputs/give-it-up-chain/{uuid.uuid4().hex}.png"
    ...
    out_path = out_dir / f"give_it_up_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument("--driving-video", default=GIVE_IT_UP_DRIVING_VIDEO,
