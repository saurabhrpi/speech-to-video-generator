"""V2 Pipeline A Speed template — NBP-edit -> Kling Motion Control chain (S83).

Tiktok_dances category template (Diljit Dosanjh "Speed" Punjabi-rap track).
Sister to scripts/test_copacabana_chain.py (Pattern B — holistic regen,
DIFFERENT subject).

Source: competitor-app screen recording of a young South Asian man dancing on
an empty suburban street at NIGHT — dark gray pullover hoodie, black cargo
pants, white/black sneakers. Streetlight, crescent moon, distant building.
Static camera, ~1:1 aspect (1282x1386), 15s after a 0.5s start-trim.

Note: the source scene (empty street at night) overlaps the existing High School
template (night parking lot). Per S83 user direction the regen moves him to a
DISTINCT night scene — an EMPTY HIGHWAY at night (open road, vanishing-point
lane markings, distant car taillights, asphalt) — keeping the night-streetwear
register, new identity, plays on the song's name.

Pattern B holistic regen:
- Subject: DIFFERENT South Asian man, dark hair, athletic build (same ethnic
  register as the source, distinct identity).
- Wardrobe: black pullover hoodie + olive cargo trousers + chunky sneakers
  (source = dark GRAY hoodie + BLACK cargo). Same streetwear silhouette,
  swapped color register.
- Scene: open HIGHWAY at night, NOT a suburban side-street or parking lot.

Night handling (S80 Like a G6 lesson): scene REGEN-framed as a genuine
night-on-highway photograph lit by warm headlights / distant taillights / moon
— NOT a darkened daylight subject.
...
Usage:
    .venv/bin/python scripts/test_speed_chain.py --no-kling
    .venv/bin/python scripts/test_speed_chain.py --edited-image ~/Downloads/speed_edit_xxx.png --keep-audio
"""
...
DEFAULT_REFERENCE = Path(
    "/Users/saurabhsmacbookair/Downloads/App Templates Prep/speed_first_frame.png"
)

# Pattern B — holistic regen, DIFFERENT subject. South Asian man, dark hair,
# athletic build, on an empty HIGHWAY at night (distinct from source's
# suburban street). Wardrobe = black hoodie + olive cargo (source = dark gray
# hoodie + black cargo). Streetwear silhouette preserved, color register
# swapped.
SPEED_EDIT_PROMPT = (
    "Edit this image:\n"
    "- Replace the subject with a DIFFERENT young South Asian man — "
    "early-to-mid 20s, athletic build (broad shoulders, lean frame), with "
    "short dark hair (modern fade or textured crop), warm medium-brown skin, "
    "neat short stubble OR clean-shaven, a confident relaxed expression. He "
    "must be clearly a different person from the one in the input (different "
    "facial features, different identity from the input's mustache + soft "
    "beard). Standing upright facing the camera, full body visible from head "
    "to feet.\n"
    "- Wardrobe: a fitted BLACK pullover hoodie + OLIVE / sage-green cargo "
    "trousers with deep side pockets + chunky white-or-black streetwear "
    "sneakers. NOT the input's DARK-GRAY hoodie + BLACK cargo. Same "
    "streetwear silhouette, distinct color register. NO brand logos or "
    "readable text; any graphic must be a small generic abstract design.\n"
    "- Scene: an EMPTY two-lane HIGHWAY at night — open asphalt road with "
    "clear white lane markings receding toward a vanishing point, distant "
    "warm car taillights (small red glow far away), low horizon, dark sky "
    "with a faint moon or a few stars, soft sodium / streetlamp glow along "
    "the shoulder. CRITICAL: a DIFFERENT scene from the input's suburban "
    "side-street register — NO sidewalk trees / bushes flanking close on "
    "both sides, NO distant building, NO street parking. Open road, broader "
    "horizon, vanishing-point geometry. He must read as GENUINELY "
    "photographed on a highway at night, with consistent low-key warm "
    "lighting on his skin and hoodie — do NOT render a brightly daylit "
    "subject composited onto a dark background. Cinematic late-night "
    "highway vibe.\n"
    "- Preserve the input's framing approximately — a medium-wide shot with "
    "feet visible and a clear margin of asphalt below them. Subject centered "
    "horizontally, occupying roughly the central 55-70% of the frame height. "
    "Do NOT crop the feet or the top of his head.\n"
    "- FEET AND FLOOR (critical): show the feet FULLY with a clear margin of "
    "asphalt below them — do NOT crop the feet at the bottom edge. Keep feet "
    ...
    "naturally be behind it (the highway / night sky).\n"
...
SPEED_DRIVING_VIDEO = (
    "https://assets.speech-2-video.ai/viral-dances/speed/raw_source.mp4"
)
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "speed_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False) -> int:
    ...
    key = f"spike-outputs/speed-chain/{uuid.uuid4().hex}.png"
    ...
    log.info(
        ...
        SPEED_DRIVING_VIDEO, KLING_MODEL_NAME, KLING_CHARACTER_ORIENTATION, KLING_MODE, keep_audio,
    )
    ...
    out_path = out_dir / f"speed_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument(
        ...
        help=f"Path to Speed reference image (default: {DEFAULT_REFERENCE})",
    )
    ap.add_argument(
        "--edited-image",
