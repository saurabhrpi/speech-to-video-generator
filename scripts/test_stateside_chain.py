"""V2 Pipeline A Stateside template — NBP-edit → Kling Motion Control chain (S80).

Ninth template of the "Girl Dances" row (category girl_dances).
Sister to scripts/test_woman_chain.py.
...
Source: stateside_start_at_1_sec.mov (1284×1392 near-square, ~16s; trim the
first 1s per filename → ~15s driver). Reference frame = trimmed clip t=0
(stateside_ref.png) — the actual driving start frame.

Usage:
    .venv/bin/python scripts/test_stateside_chain.py --no-kling
    .venv/bin/python scripts/test_stateside_chain.py --edited-image ~/Downloads/stateside_edit_xxx.png --keep-audio
"""
...
DEFAULT_REFERENCE = Path(
    "/Users/saurabhsmacbookair/Downloads/App Templates Prep/stateside_ref.png"
)

STATESIDE_EDIT_PROMPT = (
    "Edit this image:\n"
...
STATESIDE_DRIVING_VIDEO = (
    "https://assets.speech-2-video.ai/viral-dances/stateside/raw_source.mp4"
)
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "stateside_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False,
              driving_video: str = STATESIDE_DRIVING_VIDEO) -> int:
    settings = get_settings()
    selfies_bucket = settings.r2_selfies_bucket
    key = f"spike-outputs/stateside-chain/{uuid.uuid4().hex}.png"
    ...
    out_path = out_dir / f"stateside_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument("--driving-video", default=STATESIDE_DRIVING_VIDEO,
