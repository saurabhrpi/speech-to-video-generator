"""Seed the Firestore template registry with the Soda Pop Moves fixture (S84).

Seventeenth template of the tiktok_dances category. Third Soda Pop variant
after viral-dances-soda-pop-baby-dance and viral-dances-soda-pop-energy.
Sister to scripts/seed_luku_template.py / scripts/seed_speed_template.py.
...
with long DARK brunette hair in an upscale HOTEL LOBBY (polished marble floor,
warm-glow chandelier, marble columns), wearing a single-piece cocktail mini
dress. Distinct from the source's two-piece cami + pencil skirt and from the
other Soda Pop variants' registers. User-approved on first roll — no audio
shift applied (Kling output landed in sync on this gen; per-roll variable per
...
the explicit "55-65% frame height, generous open marble floor on both sides,
...
Re-running overwrites the Soda Pop Moves doc.

Usage:
    .venv/bin/python scripts/seed_soda_pop_moves_template.py
"""
...
log = logging.getLogger("seed_soda_pop_moves_template")


TEMPLATE_ID = "viral-dances-soda-pop-moves"
...
_BASE = "https://assets.speech-2-video.ai/viral-dances/soda-pop-moves"

SODA_POP_MOVES_FIXTURE = {
    "pipeline_class": PIPELINE_MOTION_TRANSFER,
    "outcome": OUTCOME_ONTO_CHARACTER,
    "category": "tiktok_dances",
    "title": "Soda Pop Moves",
    "description": "Lobby-glam after dark. 🥂✨",
...
def main() -> None:
    ...
    written = upsert_template(TEMPLATE_ID, SODA_POP_MOVES_FIXTURE)
    log.info(
