"""V2 Pipeline A Copacabana template — NBP-edit -> Kling Motion Control chain (S83).

Tiktok_dances category template (Barry Manilow "Copacabana" lounge dance).
Sister to scripts/test_seteadora_chain.py / test_7_11_chain.py (Pattern B —
holistic regen, DIFFERENT subject).

Source: competitor-app screen recording of a young man (white, dark hair, slim)
in a wood-paneled gentleman's-club / pub interior — white long-sleeve crew tee,
dark-wash jeans. He dances Copacabana-style synchronized lounge moves: hand /
arm chest-level waves, hand-on-hip Latin stance, twirls. Face visible most of
the clip. Static camera, ~1:1 aspect (1284x1394), 9.6s after a 0.5s
start-trim.

Pattern B holistic regen — DIFFERENT subject, DIFFERENT scene, DIFFERENT
wardrobe register from the source:
- Subject: Latino man, dark hair, athletic build (matches Latin nightclub
  cultural register of the Copacabana song).
- Wardrobe: fitted black silk shirt + slim gray trousers (source = WHITE
  long-sleeve crew + dark jeans). Dressy Tony-Manero-leaning lounge silhouette.
- Scene: upscale jazz / cocktail lounge — low warm light, tufted leather
  banquettes, brass fixtures, intimate cocktail tables, dim spots. Close to
  source register (lounge interior) but DISTINCT — cocktail-bar mood, NOT a
  pub / gentleman's club.
...
Usage:
    .venv/bin/python scripts/test_copacabana_chain.py --no-kling
    .venv/bin/python scripts/test_copacabana_chain.py --edited-image ~/Downloads/copacabana_edit_xxx.png --keep-audio
"""
...
DEFAULT_REFERENCE = Path(
    "/Users/saurabhsmacbookair/Downloads/App Templates Prep/copacabana_first_frame.png"
)

# Pattern B — holistic regen, DIFFERENT subject. Latino man, dark hair, athletic
# build, in an upscale jazz/cocktail lounge (distinct from source's pub/
# gentleman's-club). Wardrobe = black silk shirt + slim gray trousers (Tony
# Manero / dressy lounge register, distinct from source's white casual).
COPACABANA_EDIT_PROMPT = (
    "Edit this image:\n"
    "- Replace the subject with a DIFFERENT young man — Latino, early-to-mid "
    "20s, athletic build (toned arms / shoulders), with short-to-medium dark "
    "hair (clean modern cut), warm tan skin, a confident relaxed expression. "
    "Clean-shaven or with a short, neat stubble. He must be clearly a "
    "different person from the one in the input. Standing upright facing the "
    "camera, full body visible from head to feet.\n"
    "- Wardrobe: a fitted BLACK silk (or satin-sheen) button-up shirt with "
    "the top one or two buttons undone, tucked into slim slate / charcoal "
    "GRAY trousers, with a thin belt and clean leather dress shoes. NOT the "
    "input's WHITE long-sleeve crew tee + DARK-WASH JEANS. Dressy lounge "
    "silhouette. NO brand logos or readable text; any graphic must be a small "
    "generic abstract design.\n"
    "- Scene: an upscale jazz / cocktail lounge interior — low warm "
    "incandescent light, tufted leather banquettes in deep oxblood or "
    "espresso, brass railings and fixtures, intimate cocktail tables with "
    "small lit candles, soft amber spotlights, a hint of distant patrons "
    "blurred out of focus. CRITICAL: a DIFFERENT lounge composition from the "
    "input's pub / gentleman's-club register — NO green tufted booths, NO "
    "framed black-and-white prints on a wood-paneled wall, NO round dining "
    "table. Moody cocktail-bar vibe, evening light. He must read as "
    "GENUINELY photographed inside a dim cocktail lounge, with consistent "
    "low-key warm lighting on his skin and shirt — do NOT render a brightly "
    "lit subject composited onto a dark background.\n"
    ...
    "naturally be behind it (the cocktail lounge interior).\n"
...
COPACABANA_DRIVING_VIDEO = (
    "https://assets.speech-2-video.ai/viral-dances/copacabana/raw_source.mp4"
)

# Kling config (S73 defaults): v2.6 + pro + video-orientation (driving ~9.6s).
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "copacabana_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False) -> int:
    ...
    key = f"spike-outputs/copacabana-chain/{uuid.uuid4().hex}.png"
    ...
    log.info(
        ...
        COPACABANA_DRIVING_VIDEO, KLING_MODEL_NAME, KLING_CHARACTER_ORIENTATION, KLING_MODE, keep_audio,
    )
    ...
    result = client.generate_and_poll(
        image_url=image_url,
        video_url=COPACABANA_DRIVING_VIDEO,
    ...
    out_path = out_dir / f"copacabana_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument(
        ...
        help=f"Path to Copacabana reference image (default: {DEFAULT_REFERENCE})",
    )
    ap.add_argument(
        "--edited-image",
