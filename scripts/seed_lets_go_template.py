"""Seed the Firestore template registry with the Let's Go fixture (S81).

First template of the new **tiktok_dances** category. Sister to
scripts/seed_buttons_template.py (the canonical seed). Pipeline A
(motion-onto-character via Kling Motion Control), use_nbp_regen=true with the
generic full-body framing hint.

Category: tiktok_dances. The mobile label override CATEGORY_LABEL_OVERRIDES in
mobile/app/index.tsx maps it to "TikTok Dances" (mechanical title-case would give
"Tiktok Dances").

Preview note (S81): preview generated kling-v2-6 + pro + video orientation.
Subject = different young Black woman (Pattern B — source was an identifiable
real creator), box braids, grey cropped hoodie + black joggers + white sneakers,
on the source's outdoor basketball court (kept register, S81 user direction).
Source trimmed from t=0.5s to end (~14.3s), static camera.
...
Re-running overwrites the Let's Go doc.

Usage:
    .venv/bin/python scripts/seed_lets_go_template.py
"""
...
log = logging.getLogger("seed_lets_go_template")


TEMPLATE_ID = "viral-dances-lets-go"
...
_BASE = "https://assets.speech-2-video.ai/viral-dances/lets-go"
...
def main() -> None:
    ...
    written = upsert_template(TEMPLATE_ID, LETS_GO_FIXTURE)
    log.info(
