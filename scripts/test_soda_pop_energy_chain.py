"""V2 Pipeline A Soda Pop Energy template — NBP-edit -> Kling Motion Control chain (S82).

Tenth template of the tiktok_dances category. Sister to
scripts/test_dale_pa_ve_chain.py (Pattern B — holistic regen, DIFFERENT subject).
A distinct "Soda Pop" variant from viral-dances-soda-pop-baby-dance (different
song-section / source; this one is a man in a field).

Source: competitor-app screen recording of a young man dancing in a green
COUNTRYSIDE field (trees, a garden, a stone ledge), daytime soft light, cream tee
+ dark jeans, static-ish camera, a tighter medium shot (feet cropped), ~9.0s
after a 0.5s start-trim. S82 user direction: keep it a man (new identity) in the
daytime countryside field; full-body framing (push feet into frame since the
source crops them).

Daytime scene (no night-relight needed).
...
Usage:
    .venv/bin/python scripts/test_soda_pop_energy_chain.py --no-kling
    .venv/bin/python scripts/test_soda_pop_energy_chain.py --edited-image ~/Downloads/soda_pop_energy_edit_xxx.png --keep-audio
"""
...
DEFAULT_REFERENCE = Path(
    "/Users/saurabhsmacbookair/Downloads/App Templates Prep/Working/soda_pop_energy_ref.png"
)

# Pattern B — holistic regen, DIFFERENT subject (S82). Keep it a man in a daytime
# countryside field, casual register. Source is a tighter medium shot that crops
# the feet -> push a wider full-body framing with feet visible. Daytime soft light
# (no night-relight). Hybrid-permissive menus per feedback_nbp_hybrid_default.md.
SODA_POP_ENERGY_EDIT_PROMPT = (
    "Edit this image:\n"
    "- Replace the subject with a DIFFERENT young man — fair / light skin tone, "
    "similar age (approximately early-to-mid 20s), athletic slim build, with a "
    "relaxed confident smile. He must be clearly a different person from the one "
    "in the input (different facial features, different identity). Different hair "
    "from the input (e.g. short tousled light-brown hair, a tidy crop, or longer "
    "wavy hair — pick one); light stubble optional. Same general upright standing "
    "dance pose, facing the camera, full body visible from head to feet.\n"
    "- Wardrobe: casual countryside streetwear in a similar register to the input "
    "but not identical — e.g. a plain cream crew tee, an off-white henley, or a "
    "light short-sleeve overshirt, paired with dark olive or brown slim jeans or "
    "chinos and casual boots or sneakers — pick one cohesive look. NO brand logos "
    "or readable text; any graphic must be a small generic abstract design.\n"
    "- Scene: a sunlit green countryside field / meadow with scattered trees, "
    "rolling hills, and a soft daytime sky. Similar register to the input but a "
    "distinct composition. Genuine soft daytime outdoor lighting on the subject. "
    "Relaxed rural-summer vibe.\n"
    "- Frame him as a WIDER full-body shot — FARTHER from the camera and SMALLER "
    "in the frame than the input (the input is a tight medium shot; pull WAY "
    "back), occupying roughly the central 55-65% of the frame height, and "
    "CENTERED horizontally. Leave GENEROUS, roughly EQUAL open field on BOTH the "
    "left and right sides, plus clear ground below the feet — room for lateral "
    "dance movement and a full arm wingspan in either direction. Nothing within "
    "arm's reach on either side.\n"
    "- FEET AND FLOOR (critical): the input crops the feet — instead show the "
    "feet FULLY with a clear margin of ground below them. Do NOT crop the feet at "
    "the bottom edge. Keep feet >=8-10% of frame height above the bottom.\n"
    ...
    "buttons, no text overlays, no status icons. Just the subject in the field."
)
...
SODA_POP_ENERGY_DRIVING_VIDEO = (
    "https://assets.speech-2-video.ai/viral-dances/soda-pop-energy/raw_source.mp4"
)

# Kling config (S73 defaults): v2.6 + pro + video-orientation (driving ~9.0s).
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "soda_pop_energy_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False) -> int:
    ...
    key = f"spike-outputs/soda-pop-energy-chain/{uuid.uuid4().hex}.png"
    ...
    log.info(
        ...
        SODA_POP_ENERGY_DRIVING_VIDEO, KLING_MODEL_NAME, KLING_CHARACTER_ORIENTATION, KLING_MODE, keep_audio,
    )
    ...
    result = client.generate_and_poll(
        image_url=image_url,
        video_url=SODA_POP_ENERGY_DRIVING_VIDEO,
    ...
    out_path = out_dir / f"soda_pop_energy_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument(
        ...
        help=f"Path to Soda Pop Energy reference image (default: {DEFAULT_REFERENCE})",
    )
    ap.add_argument(
        "--edited-image",
