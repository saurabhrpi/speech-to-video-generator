"""Seed the Firestore template registry with the Bad fixture (S71).

Sister to scripts/seed_beat_it_template.py. Pipeline A (motion-onto-character
via Kling Motion Control), use_nbp_regen=true with the same generic full-body
framing hint we use for Bombale, Gangsta, Baby Dance, and Beat It.

Category: mj_dances (with Beat It and the eventual Smooth Criminal).
...
Re-running overwrites the Bad doc.

Usage:
    .venv/bin/python scripts/seed_bad_template.py
"""
...
log = logging.getLogger("seed_bad_template")


TEMPLATE_ID = "viral-dances-bad"
...
BAD_FIXTURE = {
    ...
    "category": "mj_dances",
    "title": "Bad",
    "description": "Sharp moves, bad attitude. 🔥💃",
    "published_status": STATUS_DRAFT,
    "assets": {
        "driving_video_url": "https://assets.speech-2-video.ai/viral-dances/bad/driving_video.mp4",
        "scene_image_url": None,
        "thumbnail_url": None,
        "preview_video_url": "https://assets.speech-2-video.ai/viral-dances/bad/preview_video.mp4",
    },
...
def main() -> None:
    ...
    written = upsert_template(TEMPLATE_ID, BAD_FIXTURE)
    log.info(
