"""V2 Pipeline A Lush Life template — NBP-edit -> Kling Motion Control chain (S82).

Seventh template of the tiktok_dances category. Sister to
scripts/test_freeze_chain.py (Pattern B — holistic regen, DIFFERENT subject;
source is an identifiable real creator).

Source: competitor-app screen recording of a light-skinned young woman dancing on
a sandy BEACH at sunset / golden hour (calm sea, warm pink sky, distant
shoreline), loose white button-up shirt + light high-waist denim shorts, static
camera, ~15.0s after a 0.5s start-trim. S82 user direction: keep the beach-sunset
scene + summery white-shirt / denim-shorts register, new light-skinned identity.

Bright/warm outdoor dusk scene (no night-relight needed). The source crops the
feet near the bottom edge — the regen pushes a roomier full-body framing with
feet fully visible.
...
Usage:
    .venv/bin/python scripts/test_lush_life_chain.py --no-kling
    .venv/bin/python scripts/test_lush_life_chain.py --edited-image ~/Downloads/lush_life_edit_xxx.png --keep-audio
"""
...
DEFAULT_REFERENCE = Path(
    "/Users/saurabhsmacbookair/Downloads/App Templates Prep/Working/lush_life_ref.png"
)

# Pattern B — holistic regen, DIFFERENT subject (S82). Keep the register: a
# light-skinned young woman dancing on a dusk beach in a summery white-shirt +
# denim-shorts look. Warm golden-hour scene (no night-relight). S78 roomy
# composition + explicit feet-visible (source crops feet). Hybrid-permissive
# menus per feedback_nbp_hybrid_default.md.
LUSH_LIFE_EDIT_PROMPT = (
    "Edit this image:\n"
    "- Replace the subject with a DIFFERENT young woman — fair / light skin "
    "tone, similar age (approximately early-to-mid 20s), slim build, with a "
    "joyful confident expression. She must be clearly a different person from the "
    "one in the input (different facial features, different identity). Different "
    "hairstyle AND hair color from the input (e.g. sun-kissed blonde, warm light "
    "brown, or soft auburn — pick one), long, in loose beach waves, sleek "
    "straight, or a half-up style. Dainty earrings optional. Same general "
    "upright standing dance pose, facing the camera, full body visible from head "
    "to feet.\n"
    "- Wardrobe: a summery beach outfit in the same casual-chic register as the "
    "input but a DIFFERENT outfit — do NOT use a white shirt with denim shorts. "
    "Instead a flowy summer dress, e.g. a light floral midi sundress, a pastel "
    "spaghetti-strap slip dress, or a white crochet / knit beach dress — pick one "
    "cohesive look. Barefoot or simple flat sandals. NO brand logos or readable "
    "text; any graphic must be a small generic abstract design.\n"
    "- Scene: a sandy beach at sunset / golden hour — soft warm pink-and-gold "
    "sky, calm sea with gentle waves on the horizon, a distant low shoreline. "
    "Similar register to the input but a distinct composition. Genuine warm "
    "outdoor dusk lighting on the subject, exposed consistently with the sky. "
    "Relaxed summer-evening vibe.\n"
    ...
    "sand on BOTH the left and right sides, plus clear sand below the feet — room "
    "for lateral dance movement and a full arm wingspan in either direction. "
    "Nothing within arm's reach on either side.\n"
    "- FEET AND FLOOR (critical): the input crops the feet near the bottom edge — "
    "do NOT do that. Show the feet FULLY with a clear margin of sand below them. "
    "Keep feet >=8-10% of frame height above the bottom.\n"
    ...
    "buttons, no text overlays, no status icons. Just the subject on the beach."
)
...
LUSH_LIFE_DRIVING_VIDEO = (
    "https://assets.speech-2-video.ai/viral-dances/lush-life/raw_source.mp4"
)
...
def run_edit(client: genai.Client, reference: Path, out_dir: Path) -> tuple[int, str]:
    ...
    ok, info, _ = _save_image(resp, out_dir, "lush_life_edit")
    if ok:
...
def run_kling(edited_image_path: str, out_dir: Path, keep_audio: bool = False) -> int:
    ...
    key = f"spike-outputs/lush-life-chain/{uuid.uuid4().hex}.png"
    ...
    log.info(
        ...
        LUSH_LIFE_DRIVING_VIDEO, KLING_MODEL_NAME, KLING_CHARACTER_ORIENTATION, KLING_MODE, keep_audio,
    )
    ...
    result = client.generate_and_poll(
        image_url=image_url,
        video_url=LUSH_LIFE_DRIVING_VIDEO,
    ...
    out_path = out_dir / f"lush_life_chain_{uuid.uuid4().hex[:8]}.mp4"
...
def main():
    ...
    ap.add_argument(
        ...
        help=f"Path to Lush Life reference image (default: {DEFAULT_REFERENCE})",
    )
    ap.add_argument(
        "--edited-image",
