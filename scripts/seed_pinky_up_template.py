"""Seed the Firestore template registry with the Pinky Up fixture (S81).

Second template of the tiktok_dances category. Sister to
scripts/seed_lets_go_template.py / the canonical scripts/seed_buttons_template.py.
...
identifiable real creator), pale-blue track set on a sage-green wall (S81 user
direction: keep light-skinned woman, different light wall + fresh athletic wear,
source Adidas/Nike logos stripped). Source trimmed from t=1s to end (~15.2s),
static camera.
...
Re-running overwrites the Pinky Up doc.

Usage:
    .venv/bin/python scripts/seed_pinky_up_template.py
"""
...
log = logging.getLogger("seed_pinky_up_template")


TEMPLATE_ID = "viral-dances-pinky-up"
...
_BASE = "https://assets.speech-2-video.ai/viral-dances/pinky-up"
...
def main() -> None:
    ...
    written = upsert_template(TEMPLATE_ID, PINKY_UP_FIXTURE)
    log.info(
