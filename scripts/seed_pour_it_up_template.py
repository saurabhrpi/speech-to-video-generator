"""Seed the Firestore template registry with the Pour It Up fixture (S79).

Sister to scripts/seed_give_it_up_template.py. Pipeline A
(motion-onto-character via Kling Motion Control), use_nbp_regen=true with the
generic full-body framing hint.

Category: girl_dances. Sixth girl_dances row template.

Preview note (S79): preview generated kling-v2-6 + pro + video orientation.
Subject = different young woman (Pattern B), matched the source's tank+jeans
style/coverage exactly (user decision: keep same style, do not conservatize) in
new colors (deep-red tank + light-blue jeans + boots), spacious open night
plaza (S78 roomy composition). 13.6s, no end-trim (clean tail).
...
Re-running overwrites the Pour It Up doc.

Usage:
    .venv/bin/python scripts/seed_pour_it_up_template.py
"""
...
log = logging.getLogger("seed_pour_it_up_template")


TEMPLATE_ID = "viral-dances-pour-it-up"
...
_BASE = "https://assets.speech-2-video.ai/viral-dances/pour-it-up"
...
def main() -> None:
    ...
    written = upsert_template(TEMPLATE_ID, POUR_IT_UP_FIXTURE)
    log.info(
