...
| High-bitrate Kling output | `.../viral-dances/<slug>/driving_video.mp4` | `driving_video_url` — **the runtime Kling driver**; the high-bitrate **twin of `preview_stream.mp4`** (SAME content at two bitrates, corrected identically if applicable — NOT `raw_source.mp4`) |
| Stream preview (~5 Mbps + faststart) | `.../viral-dances/<slug>/preview_stream.mp4` | `preview_video_url` — **what the app plays** |
| First-frame poster | `.../viral-dances/<slug>/thumbnail.jpg` | `thumbnail_url` — **auto-generated by the seed** (S81) |
...
Tile: the home grid renders `assets.preview_video_url ?? assets.driving_video_url` as a looping `<Video>` (`mobile/app/index.tsx:TemplateTile`), but only mounts/plays it while on-screen (to bound iOS decoders, S80). Off-screen / not-yet-mounted tiles show `thumbnail_url` as a static `<Image>` poster — so the grid is never black. `preview_video_url` is the ~5 Mbps `preview_stream.mp4` — **keep it that bitrate**: raw Kling output is 14-35 Mbps @ 1440² and stutters/rebuffers on mobile (S77). The thumbnail (frame 0 of the preview) is now **auto-generated by the seed** (S81, step 10) — a new template no longer ships posterless.
...
**Invariant (S82): `driving_video.mp4` and `preview_stream.mp4` are the SAME Kling-output content at two bitrates.** `driving_video.mp4` is the **high-bitrate twin of the app preview** (corrected if applicable) — NOT the raw `raw_source.mp4`. `raw_source.mp4` is the separate original competitor clip (Kling's *input* / revert target), at a different resolution and length. So they always match in duration + framing; if a post-Kling correction (audio shift, head/tail trim — step 8b) is applied to one, it MUST be applied to the other. Verify with `ffmpeg -i <url>`: a template's `driving_video` and `preview_stream` durations should match within encode rounding; `raw_source` differs. (S82 near-miss: an audio-corrected preview shipped against an uncorrected raw-output driver would diverge AND hand runtime a lead-carrying driver — caught pre-upload on Freeze.)
...
2. Chain script drives off `raw_source.mp4`. Its high-bitrate Kling output becomes **`driving_video.mp4`** (the runtime driver) — the **high-bitrate twin of `preview_stream.mp4`** (same content, two bitrates). With no post-Kling correction, upload the raw Kling output directly. **If you ran the step-8b audio fix, upload the CORRECTED high-bitrate driver from there** (NOT the uncorrected raw output), so driver and preview stay twins.
...
⚠️ **If you correct the audio, correct BOTH the driver AND the preview — they must stay twins (S82).** The fix yields a corrected MASTER (the shifted + head-trimmed `<slug>_shift_0p50.mp4`). Per the [twin invariant](#-filename--role--never-delete-a-file-by-its-name), `driving_video.mp4` is the high-bitrate twin of `preview_stream.mp4`, so BOTH must be re-encoded from that same corrected master — at two bitrates. Encoding only the preview leaves `driving_video.mp4` as the uncorrected raw Kling output: driver and preview diverge (different length AND sync), and the runtime driver still carries the lead.

```bash
SHIFT=~/Downloads/<slug>_shift_0p50.mp4
# high-bitrate corrected DRIVER -> driving_video.mp4 (runtime driver):
"$FF" -y -ss 0.5 -i "$SHIFT" \
  -c:v libx264 -preset slow -crf 15 -pix_fmt yuv420p \
  -c:a aac -b:a 192k -movflags +faststart ~/Downloads/<slug>_driving_video.mp4
# ~5 Mbps corrected PREVIEW -> preview_stream.mp4 (what the app plays):
"$FF" -y -ss 0.5 -i "$SHIFT" \
  -c:v libx264 -profile:v high -preset medium -b:v 5M -maxrate 6M -bufsize 12M \
  -pix_fmt yuv420p -c:a aac -b:a 128k -movflags +faststart ~/Downloads/<slug>_preview_stream.mp4
```

Both files become the step-9 uploads (`driving_video.mp4` + `preview_stream.mp4`); skip the step-8 trim (already re-encoded). Confirm they came out the same duration.

**Runtime caveat (corrected S82 — the old "Kling re-applies it on every output" claim was WRONG).** The lead was only ever *confirmed* on `pro` mode (v2.6-pro S75, v3-pro S76), which is how previews are built. Current global runtime is **v2.6-`std`**, where the lead was never confirmed; the one real runtime gen ever checked (S76 DPWM) came out **synced**. Two non-disambiguated reasons it may be clean: (a) the corrected driver propagates, or (b) `std` simply doesn't introduce the lead. Either way, shipping a corrected driver (above) is never worse. A fixed per-template `audio_offset_sec` is NOT a reliable fix — the lead magnitude varies per-GEN (S76 Beat It) — and it isn't implemented (AIV-105 open). The only way to know runtime sync for a template is to **verify with one real gen**. See `Memory/feedback_kling_audio_lead_and_preview_propagation.md`.
...
**Copy `scripts/seed_buttons_template.py` verbatim** (S78/S81 canonical — has the full migrated asset shape AND the inline first-frame poster step; supersedes the older "copy `seed_baby_dance_template.py`" instruction), then change only:
...
**First-frame poster (S81) — automatic, do NOT add a manual step.** After `upsert_template`, the seed calls `generate_thumbnail(written, force=True)` (from `src/speech_to_video/utils/template_thumbnail.py`): it extracts frame 0 of `preview_stream.mp4`, uploads `thumbnail.jpg`, and sets `assets.thumbnail_url`. This is why the fixture's `thumbnail_url` stays `None` (the seed fills it) and why **`preview_stream.mp4` must already be on R2 before you run the seed** (it is — step 9). The poster step is best-effort: if it fails (e.g. R2/CDN hiccup), the seed still writes the doc and logs a warning — backfill later with `scripts/generate_template_thumbnails.py --template-id viral-dances-<slug>` (that tool only sees PUBLISHED templates, so backfill after step 13).
...
Writes the doc with `STATUS_DRAFT` — the template won't appear in the mobile gallery yet (gallery filters to `published`). The run also logs `thumbnail: ok` and a read-back line showing `thumbnail=thumbnail.jpg` — confirm both (a `thumbnail: failed`/`skipped` means the poster wasn't produced; check that `preview_stream.mp4` is on R2, then backfill per step 10's note).
...
**Poster gate (S81):** publishing requires a usable `assets.thumbnail_url`. The seed (step 10) already set it, so this normally passes silently. If it's missing (e.g. the seed's poster step failed, or you seeded with an older no-thumbnail seed), the publish command generates the poster first; it refuses with **exit code 3** only if `preview_stream.mp4` isn't on R2 yet. In that case, fix the preview upload (step 9) or backfill with `scripts/generate_template_thumbnails.py --template-id viral-dances-<slug>`, then retry. The gate is enforced regardless of how the doc was seeded, so no template can ship a black tile.
...
- **Cleaning the scratch folder.** The source/intermediate files this runbook drops into `~/Downloads/App Templates Prep/` pile up. To prune them safely once templates are live, follow `docs/template_prep_cleanup_runbook.md` — it verifies each product is live on R2 (HEAD 200) before treating local files as redundant, and never deletes by filename.
