...
- **Driver upscale (optional).** If a gen looks soft/low-detail, the driver may be too low-res — upscale the raw source, then re-run the chain with `--driving-video <upscaled URL>` (S78 arg). NOT a default step.
  - **Preferred: Topaz Video API** — `scripts/upscale_topaz.py`, runs on Topaz's cloud (~1 min actual despite a ~5 min estimate; vs Replicate's ~60 min). Needs `TOPAZ_API_KEY` in `.env`. `POST` create returns a FREE cost estimate first (`--stop-after create`). Upscales resolution only (no fps change) + `audioTransfer:Copy` keeps audio synced → **NO re-time/re-mux needed**. Default model `prob-4` (Proteus v4). Full API facts in the `scripts/upscale_topaz.py` docstring + the OpenAPI spec `docs/api-notes/topaz-video-api.yaml`.
    - **Cost (confirmed S79):** a 15s 2× upscale = **9 credits / ~$1.08 actual** (low end of the 9-10 estimate). Find the ACTUAL charge via `GET /video/history` → `transactions` (sum `commit` − `rollback`); the status/create endpoints only return the estimate. Full method in `Memory/reference_topaz_video_api.md`.
    - **⚠️ Re-encode before feeding Kling.** Topaz output is ~90 Mbps (≈178 MB for 15s @ 2k) — Kling Motion Control **rejects** it (`code 1201 "Video size is too large"`, S79 River). Re-encode to ~14 Mbps first (keeps 2k; The Hills' working 2k driver was ~19 Mbps / 36 MB), THEN `--driving-video <re-encoded URL>`:
      ```bash
      "$FF" -y -i topaz_2k.mp4 -c:v libx264 -preset slow -b:v 14M -maxrate 18M -bufsize 28M \
        -pix_fmt yuv420p -movflags +faststart -c:a copy driver_2k_14m.mp4
      ```
    - Upscaling sharpens detail/hands on an **already-trackable** driver; it does NOT rescue an untrackable (floor/occluded) source — see `Memory/reference_kling_min_continuous_valid_motion.md`. fps does NOT need normalizing (the River 30fps experiment was a red herring).
