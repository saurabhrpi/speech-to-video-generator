...
- **Preview vs. runtime Kling config split (S73-locked).** Chain scripts: `pro` mode + `kling-v2-6` (catalog-quality preview, ~$1.50 Kling-side). Runtime call sites in `video_service.py:969` + `:1126`: `std` mode + `kling-v2-6` (~$0.50 Kling-side). See `Memory/project_kling_mode_split.md`. Don't accidentally flip runtime to `pro` — it doubles per-gen cost. Future consideration: bump preview-only to `kling-v3` for facial-consistency; spike A/B before flipping.
