...
**Lead magnitude VARIES per gen/roll — NOT a fixed constant (S76 Beat It).** The +0.5s that fixed DPWM under-corrected one Beat It Kling roll and over-corrected the next (same template + driver, different rolls). So you can't assume a fixed offset across templates OR even across re-rolls of the same template. Implication: a fixed `-itsoffset` — and the per-template fixed `audio_offset_sec` proposed in AIV-105 — can't reliably fix sync, because the correct offset is effectively per-GEN. This makes a robust automated runtime fix hard. Combined with the recurring [[kling-end-dup-hallucination]], it's why the S76 Beat It preview rebuild was abandoned and the prod version retained.
