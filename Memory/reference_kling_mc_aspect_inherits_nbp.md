...
**⚠️ Lateral room must be SPATIAL, not just aspect (S83 — Copacabana):** A perfectly 1:1 NBP edit can STILL be cramped if scene props/walls/booths/crowd flank the subject within arm's reach — the aspect is right but the subject has no in-canvas lateral allowance. Symptoms in the Kling output when this happens:
- **Cramped dance** — arms clip frame edges or push against background objects during lateral choreography (S78 Buttons / The Hills original finding).
- **Face distortion (S83 NEW finding)** — Kling's motion-transfer also *warps the subject's face* during high-motion sections when the frame can't accommodate the lateral motion the dance demands. The face stretches / smears / re-renders inconsistently across frames, even if the body fits. Confirmed S83 Copacabana: first roll had tufted-leather booths within ~1m of the subject on both sides; output showed both cramped arm-waves AND mid-clip face distortion. Pattern A re-edit pushing the booths to the frame edges + adding generous open floor fixed BOTH symptoms in the re-run.

**Bake the lateral-room clause into every Pattern B chain prompt:** *"Frame as a WIDER full-body shot — FARTHER from the camera and SMALLER in the frame than the input, occupying roughly the central 55-65% of the frame height. Leave GENEROUS, roughly EQUAL open empty floor on BOTH the left and right sides — room for lateral dance movement in either direction. Nothing within arm's reach on either side."* When a chain produced a cramped edit (booths/walls/people close to subject), Pattern A re-edit prompt: *"Push the [props/walls/booths] FARTHER BACK and FARTHER OUT to the LEFT and RIGHT edges. Open floor must extend at least the width of the subject's full arm wingspan on each side."* The exception is the **camera-approach** case ([[kling-end-dup-hallucination]] / S73 Beat It Hubx) — if the source driver has the camera pushing IN, keep close framing to avoid duplicate-subject spawn; trim the tail downstream instead. See also [[nbp-cant-reposition]] for the size-jitter caveat — Pattern A can re-edit surroundings reliably but can't reliably *rescale* the subject; if subject scale is the problem, do a Pattern B holistic re-roll with the explicit roomy clause.

**How to apply:** Pre-template prep step — verify the NBP edit's aspect ratio matches what you want Kling to output. If aspect drift, either re-NBP with composition-steering pose prompt, or Pillow-resize the NBP edit before R2 upload (less reliable; NBP's natural composition is the cleaner fix). Separately, evaluate the driving video for camera tracking — a static-camera source with a laterally-moving dancer will produce chopped limbs even with a 1:1 character image. See related: [[regen-vs-preserve-prompts]], [[nbp-wont-remove-ui-overlays]], [[kling-driver-quality-checklist]], [[nbp-cant-reposition]], [[kling-end-dup-hallucination]].
